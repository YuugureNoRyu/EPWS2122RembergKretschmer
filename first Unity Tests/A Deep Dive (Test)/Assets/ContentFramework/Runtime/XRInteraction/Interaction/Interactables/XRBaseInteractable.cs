...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public abstract class XRBaseInteractable : NetworkingInteractable
    {
        ...
        /// <summary>
        /// Serialize this base interactable.
        /// </summary>
        ...
        protected override void SerializeView(Photon.Pun.PhotonStream stream)
        {
            ...
            {
                var hovered = (bool)stream.ReceiveNext();
                var selected = (bool)stream.ReceiveNext();

                if (hovered != isHovered)
                {
                    isHovered = hovered;
                    if (isHovered)
                        m_OnHoverEnter?.Invoke(null);
                    else
                        m_OnHoverExit?.Invoke(null);
                }
                ...
                if (selected != isSelected)
                {
                    isSelected = selected;
                    if (isSelected)
                        m_OnSelectEnter?.Invoke(null);
                    else
                        m_OnSelectExit?.Invoke(null);
                }
            }
        }
#endif
