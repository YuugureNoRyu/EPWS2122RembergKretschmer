...
namespace wovr
{
    ...
    public static class GuidManager
    {
        ...
        private static Dictionary<Guid, GuidInfo> m_GuidToObjectMap = new Dictionary<Guid, GuidInfo>();

        /// <summary>
        /// All the public API is static so you need not worry about creating an instance
        /// </summary>
        /// <param name="guidComponent"></param>
        /// <returns></returns>
        public static bool Add(GuidComponent guidComponent)
        {
            ...
            if (!m_GuidToObjectMap.ContainsKey(guid))
            {
                m_GuidToObjectMap.Add(guid, info);
                return true;
            }

            var existingInfo = m_GuidToObjectMap[guid];
            if (existingInfo.go != null && existingInfo.go != guidComponent.gameObject)
            {
                ...
                if (Application.isPlaying)
                    Debug.AssertFormat(false, guidComponent, "Guid Collision Detected between {0} and {1}.\nAssigning new Guid. Consider tracking runtime instances using a direct reference or other method.", (m_GuidToObjectMap[guid].go != null ? m_GuidToObjectMap[guid].go.name : "NULL"), (guidComponent != null ? guidComponent.name : "NULL"));
            ...
            m_GuidToObjectMap[guid] = existingInfo;
            return true;
        }

        /// <summary>
        /// Remove this guid from the manager.
        /// </summary>
        /// <param name="guid"></param>
        public static void Remove(Guid guid)
        {
            // trigger all the destroy delegates that have registered
            if (m_GuidToObjectMap.TryGetValue(guid, out GuidInfo info))
                info.HandleRemoveCallback();
            m_GuidToObjectMap.Remove(guid);
        }

        /// <summary>
        /// Resolve the given guid.
        /// </summary>
        /// <param name="guid"></param>
        ...
        {
            return ResolveGuidInternal(guid, onAddCallback, onRemoveCallback);
        }
        ...
        {
            return ResolveGuidInternal(guid, null, onDestroyCallback);
        }
        ...
        {
            return ResolveGuidInternal(guid, null, null);
        }
        ...
        /// <param name="onRemoveCallback"></param>
        /// <returns></returns>
        private static GameObject ResolveGuidInternal(Guid guid, Action<GameObject> onAddCallback, Action onRemoveCallback)
        {
            if (m_GuidToObjectMap.TryGetValue(guid, out GuidInfo info))
            {
