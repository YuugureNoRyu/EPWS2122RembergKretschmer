...
namespace UnityEngine.XR.Interaction.Toolkit
{
	...
	{        
        ...
        private List<XRBaseInteractable> m_CurrentInteractables = new List<XRBaseInteractable>();
        ...
        {
            var interactors = m_Interactors.ToArray();
            for (int i = 0; i < interactors.Length; i++)
                interactors[i].ProcessInteractor(updatePhase);
        }
           
        void ProcessInteractables(XRInteractionUpdateOrder.UpdatePhase updatePhase)
        {
            var interactables = m_Interactables.ToArray();
            for (int i = 0; i < interactables.Length; i++)
                interactables[i].ProcessInteractable(updatePhase);
        }
        ...
        public void UseAndDisposeItem()
        {
            for (int i = 0; i < m_CurrentInteractables.Count; i++)
            {
                var interactable = m_CurrentInteractables[i];
                if (interactable != null)
                {
                    interactable.ForceDeselect();
                    if (interactable.DisposeOnUse)
                    {
                ...
                {
                    m_CurrentInteractables.RemoveAt(i);
                    continue;
                }
            }
        }
        ...
        {
            m_CurrentInteractables.Remove(interactable);
